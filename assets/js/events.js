...
// Handles event filtering, animations, and interactive features
...
function initEventFiltering() {
    ...
    function shouldShowCard(cardData, activeFilters) {
        ...
        const hasActiveFilters = activeFilters.type.length > 0 || activeFilters.language.length > 0;
        if (!hasActiveFilters) {
            return true;
        }
        ...
        if (activeFilters.type.length > 0) {
            if (!activeFilters.type.includes(cardData.type)) {
                return false;
            }
        }
